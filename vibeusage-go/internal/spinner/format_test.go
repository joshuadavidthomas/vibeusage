...
func TestFormatCompletion(t *testing.T) {
	tests := []struct {
		...
		{
			"success with source and duration",
			CompletionInfo{ProviderID: "copilot", Source: "device_flow", DurationMs: 189, Success: true},
			"✓ copilot (device_flow, 189ms)",
		},
		{
			"success with seconds",
			CompletionInfo{ProviderID: "gemini", Source: "oauth", DurationMs: 1200, Success: true},
			"✓ gemini (oauth, 1.2s)",
		},
		{
			"failure",
		...
		{
			"failure with source",
			CompletionInfo{ProviderID: "cursor", Source: "api_key", DurationMs: 50, Success: false, Error: "timeout"},
			"✗ cursor (timeout)",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := FormatCompletion(tt.info)
			if got != tt.want {
				t.Errorf("FormatCompletion(%+v) = %q, want %q", tt.info, got, tt.want)
			}
		})
	}
}
